...
| 9 | MISLEADING / TRUE | 0.005 | 0.015 | 0 |
...
Ten adjudicate transactions produced the eight verdicts of the first eight matches. Two reached FINALIZED with a non-accepting result and wrote nothing to contract state:
...
Both executions ran to completion and produced labels; neither reached contract state. On match 8 the same claim drew FALSE in the discarded round and UNSUPPORTED in the round that was accepted, which is the label the contract holds. That is the sharpest evidence here about how a live jury converges under adversarial input, and it exists only because the transaction log keeps what contract state discards. [docs/resolution.md, item 4](docs/resolution.md) sets out the status and result distinction in full. Match 9 added the eleventh adjudicate transaction and nothing to this list: [`0xb5fe11c8...938e55f5`](https://explorer-bradbury.genlayer.com/tx/0xb5fe11c8a2e02d186e6a13d99b5d54ab8e220a825404ef123510ac34938e55f5) recovered from a leader timeout through an appeal and a rotation inside that one transaction, finished AGREE and wrote its verdict on the first attempt.
...
Studio Next reaches the same two sources by another route. Its node answers `eth_getLogs` with nothing, so there is no log to walk, but `sim_getTransactionsForAddress` returns the contract's whole history in one reply and the app reads it on every visit. `web/src/chain/history-studio-next.json` is the committed half, generated by `npm run snapshot:studio-next` and merged under the live listing exactly as Bradbury's index is merged under its live tail. Replay and Labs read both there too.
...
| GenLayer Studio Next | 61997 | [`0xB84f059D...b72078e0`](https://explorer-studio-dev.genlayer.com/address/0xB84f059D11FA6ea4c24f2d5c124686f4b72078e0) | [carnageapp.xyz/?net=studio-next](https://carnageapp.xyz/?net=studio-next) | The full lifecycle, proof links and convergence from the node's transaction index. No withdrawals. |
...
The node answers `eth_getLogs` with nothing there, which for a while was read as a network that could prove nothing. It can. `sim_getTransactionsForAddress` returns every transaction ever sent to the contract in one reply, each carrying its calldata, status and rotation count, and the app reads that live, so replay proof links and the Labs convergence measurement both work. Alongside it the app ships a committed index, `web/src/chain/history-studio-next.json`, regenerated with `npm run snapshot:studio-next`, because the network resets by design and the committed half is what keeps those hashes readable when the live listing comes back empty. It does not execute outbound transfers, so `claim()` is disabled and settled balances stay recorded in the contract ledger.

Both rows were checked against the live networks on 2026-09-16.
...
- **Tests.** 128 direct-mode contract tests for `carnage.py`, 50 of them pinning audit findings closed, and 131 for the Studio Next fork. 453 front-end tests across 24 files.
