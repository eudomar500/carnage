...
def test_resolve_inconclusive_rejected_if_adjudicated(direct_vm, direct_deploy, direct_alice, direct_bob, direct_owner):
    ...
    direct_vm.mock_llm(r"(?s)minimum acceptable price.*<claim>.*</claim>", json.dumps(json.dumps({"label": "TRUE", "reasoning": "x"})))
    direct_vm.mock_llm(r"(?s)maximum budget.*<claim>.*</claim>", json.dumps(json.dumps({"label": "TRUE", "reasoning": "x"})))
