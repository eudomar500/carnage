...
def _adjudicate_with_labels(contract, direct_vm, match_id, holder_label, buyer_label):
    direct_vm.mock_llm(HOLDER_PATTERN, json.dumps(json.dumps({"label": holder_label, "reasoning": "x"})))
    direct_vm.mock_llm(BUYER_PATTERN, json.dumps(json.dumps({"label": buyer_label, "reasoning": "x"})))
...
def test_adjudicate_schedules_settle_via_finalized_self_call(direct_vm, direct_deploy, direct_alice, direct_bob, direct_owner):
    ...
    # Through the conftest helper: the hook entry is EmitInternalMessage on this
    # runner and the calldata method key is "" rather than "method", so reading
    # the shape inline here would only re-encode a detail that moved.
    scheduled = _scheduled_calls(captured, "settle")
