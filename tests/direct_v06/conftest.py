...
This tree targets contracts/carnage_v06.py on the consensus v0.6 toolchain.
It is a sibling of tests/direct, which stays on the Bradbury contract and the
released toolchain; the two need different, incompatible SDK versions, so run
them from separate virtualenvs and never from the same one.
...
~/.cache/gltest-direct and is slow; later runs use the cache.

Three differences from tests/direct are forced by gltest 0.30 rather than by
the contract, and are marked at each site: the LLM mock payload is now
double-encoded, the _gl_call_hook entry is EmitInternalMessage rather than
PostMessage (with the calldata method key moved from "method" to ""), and the
raw message dict moved from gl.message_raw to gl.message.raw.
"""
...
def _transfers_to(captured, address):
    ...
    for entry in captured:
        # v0.3 renamed the hook entry from PostMessage to EmitInternalMessage.
        pm = entry.get("PostMessage") or entry.get("EmitInternalMessage")
...
def _warp(direct_vm, timestamp):
    ...
    # v0.3 SDK: the raw message dict moved from gl.message_raw to gl.message.raw.
    for mod_name in ("genlayer.gl", "genlayer"):
        mod = sys.modules.get(mod_name)
        if mod is None:
            continue
        raw = getattr(getattr(mod, "message", None), "raw", None)
        if isinstance(raw, dict):
            raw["datetime"] = timestamp
        legacy = getattr(mod, "message_raw", None)
        if isinstance(legacy, dict):
            legacy["datetime"] = timestamp


def _calldata_method(calldata):
    """Method name out of a captured call's calldata.

    v0.3 moved the method-call key from "method" to the empty string, the same
    rename genlayer-js 2.0 made. Both spellings are accepted so this helper is
    the only place that has to know.
    """
    if not isinstance(calldata, dict):
        return None
    return calldata.get("") or calldata.get("method")


def _scheduled_calls(captured, method):
    """Every scheduled cross-contract call to `method`, in emission order.

    Value transfers carry empty calldata and are excluded; use _transfers_to
    for those.
    """
    return [pm for pm in _post_messages(captured) if _calldata_method(pm.get("calldata")) == method]
...
def _post_messages(captured):
    ...
    """
    return [
        entry.get("PostMessage") or entry["EmitInternalMessage"]
        for entry in captured
        if "PostMessage" in entry or "EmitInternalMessage" in entry
    ]
