...
# NOTE: the blank line above is load-bearing. The GenVM parses the whole
# leading comment block as the runner spec, so a "#" line placed directly
# under the Depends header is read as part of it and the deploy fails with
# "invalid_contract runner malformed". Keep the separation.
#
# Studio Next (chain 61997) fork of carnage.py. Two kinds of change only:
# the v0.6 SDK renames, and the withdrawals_enabled flag below. Every
# constant, rubric string, guard and settlement rule is byte-identical to
# the Bradbury contract at 0xc60850c93d9AaB0e8C6c678B14b0B8db2d24337A.
#
# The runner above is the v0.3 py-genlayer runner shipped in genvm-manager
# v0.6.0-rc5 (genvm-runners-all.tar.xz, runners/py-genlayer/5j/...zip). It is
# the only py-genlayer runner in that release and it resolves on Studio Next;
# verified 2026-09-14. carnage.py's own pin,
# py-genlayer:1jb45aa8ynh2a9c9xn3b7qqh8sm5q93hwfp7jqmwsfhh8jpz09h6, is a v0.2
# legacy runner and is rejected there as "invalid_contract runner malformed",
# as is the hash the v2-dev boilerplate pins. Do not substitute the
# py-genlayer:test alias: it resolves on Studio Next but the direct-mode test
# loader cannot find it in the runner tree, and the docs forbid aliases on a
# real network.
#
# withdrawals_enabled exists because Studio Next does not execute outbound
# value transfers. A contract calling emit_transfer(value=..., on="finalized")
# finalizes successfully, records the message with the correct recipient and
# amount, and moves nothing: probe transaction
# 0x2b8be14fbde214afc364795d5e6c1677e26bd9c88daa308f2ed7f9929a0c480c reached
# status 7 / FINISHED_WITH_RETURN with triggered_transactions [] and
# messageFeesConsumed 0, and the recipient balance was unchanged. The
# use_balance=True form is not an escape: it is rejected by the runner with
# "SystemError: 6: forbidden". Scheduled self-calls, which settlement depends
# on, do execute normally there.
#
# So on that network claim() cannot pay. Rather than let it zero a party's
# credited balance and emit a transfer that never lands, a deployment with
# withdrawals_enabled=False refuses the claim outright and leaves the ledger
# intact. Deploy with True on any network that settles transfers.
...
class MatchInconclusiveRefunded(gl.chain.Event):
    def __init__(self, match_id: u256, /, **blob): ...


class MatchNoRevealResolved(gl.chain.Event):
    def __init__(self, match_id: u256, /, **blob): ...


class MatchRefundedBeforeLock(gl.chain.Event):
    def __init__(self, match_id: u256, /, **blob): ...


class MatchSettled(gl.chain.Event):
    def __init__(self, match_id: u256, /, **blob): ...


class MatchClaimed(gl.chain.Event):
    def __init__(self, match_id: u256, /, **blob): ...


class SinkTransferProposed(gl.chain.Event):
    def __init__(self, pending_sink: Address, /, **blob): ...


class SinkTransferAccepted(gl.chain.Event):
...
def _now_ts() -> int:
    ...
    return _unix(_parse_ts_checked(gl.message.raw["datetime"], "block timestamp"))
...
class Carnage(gl.contract.Contract):
    next_match_id: u256
    matches: gl.storage.TreeMap[u256, Match]
    ...
    def __init__(self, withdrawals_enabled: bool):
        ...
        # False on a network that does not execute outbound transfers; see
        # the header. Fixed at deploy: there is no setter, because flipping
        # it on a chain that still cannot pay would re-open the failure it
        # exists to prevent.
    ...
    def adjudicate(self, match_id: u256) -> dict:
        ...
        gl.contract.get_at(self.address).emit(on="finalized").settle(match_id)
    ...
    def claim(self, match_id: u256) -> u256:
        # First, before any balance is read or zeroed. On a deployment whose
        # network cannot settle outbound transfers, claim() would otherwise
        # clear the credit and emit a transfer that finalizes without moving
        # anything, destroying the balance it was meant to pay.
        if not self.withdrawals_enabled:
            raise gl.vm.UserError(
                f"{ERROR_EXPECTED} withdrawals are disabled on this deployment: "
    ...
    def _pay(self, recipient: Address, amount: u256) -> None:
        if amount == 0:
            return
        gl.contract.get_at(recipient).emit_transfer(value=amount, on="finalized")
